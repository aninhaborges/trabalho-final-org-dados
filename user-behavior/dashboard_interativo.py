...
fig, ax = plt.subplots()
sns.histplot(df_filtered["Tempo de tela ligada (horas/dia)"], bins=11, kde=True, ax=ax, color='blue')
...
fig, ax = plt.subplots()
df_filtered.groupby("Sistema Operacional")["Tempo de tela ligada (horas/dia)"].mean().plot.barh(ax=ax, color=['orange','darkblue'])
...
bars = medias.plot.bar(ax=ax, color='yellow')
...
fig, ax = plt.subplots()
df_filtered.groupby("Sistema Operacional")["Drenagem da bateria (mAh/dia)"].mean().plot.barh(ax=ax, color=['orange','darkblue'])
...
fig, ax = plt.subplots()
df_filtered.groupby("Sistema Operacional")["Uso de dados (MB/dia)"].mean().plot.barh(ax=ax, color=['orange','darkblue'])
